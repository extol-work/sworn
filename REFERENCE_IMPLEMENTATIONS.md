...
- **You are implementing a SAS binding for your own operator deployment.** Consult `extol-work/notary` (`src/sas.rs`) for the reference PDA derivation and instruction encoding, and consult `extol-work/extol-cortex` for a working example of operational concerns (RPC handling, retry queues, callback signing). The specification's [bindings/sas.md](./bindings/sas.md) is the normative source for both; the code helps translate MUST clauses into working transactions.
